...
const Welcome = () => {
  ...
  useEffect(() => {
    ...
    const obtenerTotalTrabajadores = async () => {
      try {
        const response = await fetch('http://10.10.1.1:8000/apiv2/getParameters/');
        if (!response.ok) {
    ...
    const checkUserPermissions = () => {
      ...
      if (username === 'niurvisdfp' || role === 'supervisor') {
        setShowManagementLink(true);
      } else {
        setShowManagementLink(false);
      }
    };
  ...
  return (
    <div className="welcome-container">
      ...
      <div className="welcome-message">
        ...
        {showManagementLink && (
          <Link to="/gestion" className="management-link">
            Ir a Gestión (Register)
          </Link>
        )}
      </div>
      ...
      <div className="stats-container">
        <div className="stat-card">
          ...
          <p className={loading ? 'loading' : ''}>
            {loading ? 'Cargando...' : totalTrabajadores}
          </p>
        </div>
        <div className="stat-card">
