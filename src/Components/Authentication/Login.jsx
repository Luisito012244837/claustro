    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const userData = {
                    id: result.user_id || Date.now(), // Usar user_id del backend o generar uno temporal
                    username: usernameInput,
                    role: result.role || 'user', // Usar rol del backend si está disponible
                    email: result.email || '', // Si el backend devuelve email
                };
                ...
                try {
                    ...
                    // Guardar nuevos valores (formato compatible con Redux)
                    localStorage.setItem('token', accessToken);
                    localStorage.setItem('user', JSON.stringify(userData));
                    
                    // Mantener compatibilidad con el sistema anterior si es necesario
                ...
                await Swal.fire({
                    ...
                    text: `Bienvenido ${usernameInput}.`,
                    timer: 2000,
                    showConfirmButton: false
                });

                console.log('Disparando evento personalizado...');
                
                // Evento personalizado para notificar al NavBar (mantener compatibilidad)
                const loginEvent = new CustomEvent('userLoggedIn', {
                    detail: { username: usernameInput, role: userData.role }
                });
                window.dispatchEvent(loginEvent);

                console.log('Login exitoso, Redux redirigirá automáticamente...');
                
                // No necesitamos navegar manualmente, Redux se encargará de la redirección
