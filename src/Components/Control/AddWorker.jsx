...
const [formData, setFormData] = useState({
  ...
  const handleCancel = () => {
      ...
      if (result.isConfirmed) navigate('/control');
        ...
        if (!accessToken) {
          Swal.fire({
            ...
            text: 'No se encontró un token de acceso. Por favor, inicie sesión nuevamente.',
            icon: 'error',
          });
        ...
        const data = {
          ...
          cargos: item.cargos && item.cargos.length > 0 ? 
          parseInt(item.cargos[0], 10) : null // Asumiendo que cargos es un ID único, no un array
      })),
       ...
       .map(item => ({
        ...
        id_facultad: typeof item.id_facultad === 'string' ? 
          parseInt(item.id_facultad, 10) : item.id_facultad,
        responsabilidades: item.responsabilidades && item.responsabilidades.length > 0 ? 


          item.responsabilidades.map(id => 
    ...
    if (response.status === 401) {
      // Maneja específicamente el caso de no autorizado
      Swal.fire({
        title: 'Sesión expirada',
        text: 'Su sesión ha expirado. Por favor, inicie sesión nuevamente.',
        icon: 'warning',
      });
      localStorage.removeItem('accessToken'); // Elimina el token inválido
      navigate('/login');
      return;
    }
    ...
    if (response.ok) {
      ...
      console.error("Error de la API:", errorData); // Para depuración
        ...
        <div className="form-group">
         ...
         <div className="form-group">
          ...
          <Select
            ...
            onChange={(selected) => {
              // Garantiza que solo se establezca un valor o null
              const selectedValue = selected ? selected.value : null;
              console.log("Categoría docente seleccionada:", selectedValue);
              
              setFormData(prev => ({
                ...prev,
                categoriaDocente: selectedValue
              }));
            }}
            ...
            // Asegura que no sea multi-selección
            isMulti={false}
          />
          {errors.categoriaDocente && (
            <div className="error-message">
              <strong>Por favor, selecciona una categoría docente.</strong>
            </div>
          )}
        </div>
                  ...
                  <div className="form-group" style={{ flex: 1 }}>
                    ...
                    <Select
                      ...
                      value={
                        item.cargos
                          ? options.cargos
                              ?.filter((cargo) => item.cargos.includes(cargo.id_cargo))
                              .map((cargo) => ({
                                value: cargo.id_cargo,
                                label: cargo.nombre_cargo,
                              }))
                      ...
                      placeholder="Seleccionar cargo(s)"
                      classNamePrefix="react-select"
                      isMulti
                      isClearable
                      isSearchable
                        ...
                        item.id_facultad
                          ? {
                              ...
                              label: options.facultades?.find(
                                (facultad) => facultad.id_facultad === item.id_facultad
                              )?.nombre_facultad || '',
                            }
                          : null
                      }
